...
| `xyz\oihana\schema\business\documents` | The quote → purchase order → invoice cycle: `BusinessDocument` (common parent), `Quote`, `PurchaseOrder`, `Invoice`, the `BusinessDocumentExporter`/`JsonLdExporter` export layer, plus the cross-cutting value objects `TaxDetail`, `Adjustment`, `EcoFeeRule`/`AppliedEcoFee`, `DocumentTotals`, `BusinessDocumentLine`, `PaymentSchedule`/`PaymentInstallment`. | 14 | [🇬🇧 EN](wiki/en/oihana/business-documents.md) · [🇫🇷 FR](wiki/fr/oihana/business-documents.md) |
