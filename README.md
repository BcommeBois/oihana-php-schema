...
| `xyz\oihana\schema\shipping` | Standing shipping arrangements: `DeliveryRouteAssignment` — which delivery route serves an address, on which days, in which order of passage. | 1 | [🇬🇧 EN](wiki/en/oihana/shipping.md) · [🇫🇷 FR](wiki/fr/oihana/shipping.md) |
| `xyz\oihana\schema\thesaurus` | SKOS concept trees on top of `DefinedTerm`: `ThesaurusTerm`, `Concept`, `ProductCategoryTerm`, `ProductPriceCategoryTerm`, `ConceptScheme`, `Collection`, `OrderedCollection` — `broader`/`narrower` hierarchy, notes, cross-scheme mappings, collections — the priced `DeliveryMethodTerm` and the recurring `DeliveryRouteTerm`, plus the registry layer (`ThesaurusScheme`, `ThesaurusDomain`). | 11 | [🇬🇧 EN](wiki/en/oihana/thesaurus.md) · [🇫🇷 FR](wiki/fr/oihana/thesaurus.md) |
