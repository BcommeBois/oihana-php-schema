...
| `xyz\oihana\schema\business\documents` | The quote → purchase order → invoice cycle: `BusinessDocument` (common parent), `Quote`, `PurchaseOrder`, plus the cross-cutting value objects `TaxDetail`, `Adjustment`, `EcoFeeRule`/`AppliedEcoFee`, `DocumentTotals`, `BusinessDocumentLine`, `PaymentSchedule`/`PaymentInstallment`. | 11 | [🇬🇧 EN](wiki/en/oihana/business-documents.md) · [🇫🇷 FR](wiki/fr/oihana/business-documents.md) |
