...
| `xyz\oihana\schema\thesaurus` | SKOS concept trees on top of `DefinedTerm`: `ThesaurusTerm`, `Concept`, `ProductCategoryTerm`, `ConceptScheme`, `Collection`, `OrderedCollection` — `broader`/`narrower` hierarchy, notes, cross-scheme mappings, collections. | 6 | [🇬🇧 EN](wiki/en/oihana-thesaurus.md) · [🇫🇷 FR](wiki/fr/oihana-thesaurus.md) |
