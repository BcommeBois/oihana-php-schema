 ...
 * (`uninvoicedRevenue`), its cost price (`uninvoicedCostPrice`) and its purchase
 * cost (`uninvoicedPurchaseCost`), and what is ordered and still to deliver
 * (`orderBacklog`). `revenue` only sees a sale once it is invoiced ; these series
 * say how the month stands before that, `revenue` plus `uninvoicedRevenue` is
 * what was delivered, `costPrice` plus `uninvoicedCostPrice` what it cost, and
 * `purchaseCost` plus `uninvoicedPurchaseCost` what it was bought for. A record
 * whose trade is all invoiced leaves them unset.
 *
