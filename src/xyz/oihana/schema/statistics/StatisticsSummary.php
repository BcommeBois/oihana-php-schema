 ...
 *
 * It also carries the four series of the trade that is not invoiced yet
 ...
 * `uninvoicedRevenue`, `uninvoicedCostPrice`, `uninvoicedPurchaseCost` and
 * `orderBacklog` gone. A summary of any other family leaves them unset, and they
 * are not serialized.
 *
 * ⚠️ **A summed `uninvoicedCostPrice` or `uninvoicedPurchaseCost` only prices
 * the records that carried one.** When some of the summed records hold
 * `uninvoicedRevenue` without that cost, the summed cost falls short of the
 * summed revenue, and a margin read from the two comes out too high. Whoever
 * builds the summary sums only records that carry both, or says it did not.
 *
