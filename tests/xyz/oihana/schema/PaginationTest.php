...
class PaginationTest extends TestCase
{
    ...
    public function testJsonSerializeIncludesContextAndType()
    {
        ...
        $this->assertEquals(50, $data[ Oihana::LIMIT ]);
        $this->assertArrayNotHasKey(Oihana::MIN_LIMIT, $data, 'Null properties should be omitted');
    }
    ...
    {
        $items = [ new Pagination([ Oihana::LIMIT => 50 ]) ];

        $json = json_encode($items, JSON_UNESCAPED_SLASHES);

        $this->assertEquals('[{"@type":"Pagination","@context":"https://schema.oihana.xyz","limit":50}]', $json);
    }
}
