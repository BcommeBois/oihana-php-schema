...
final class HydrateEventStatusTest extends TestCase
{
    ...
    {
        $bare = hydrateEventStatus( [ EventStatusType::SCHEDULED , EventStatusType::SCHEDULED ] ) ;

        $this->assertSame( [ EventStatusType::SCHEDULED , EventStatusType::SCHEDULED ] , $bare ) ;

        $mixed = hydrateEventStatus( [ EventStatusType::SCHEDULED , [ Schema::AT_TYPE => 'EventCancelled' ] ] ) ;

        $this->assertSame( [ 0 , 1 ] , array_keys( $mixed ) ) ;
        $this->assertSame( EventStatusType::SCHEDULED , $mixed[ 0 ] ) ;
        $this->assertInstanceOf( EventCancelled::class , $mixed[ 1 ] ) ;
    }
}
