...
- [`xyz\oihana\schema\enumerations`](../../../src/xyz/oihana/schema/enumerations) — `BusinessDocumentAuthority`, reused by `BusinessDocument::$authority`; `DocumentTotalsAccuracy`, reused by `BusinessDocument::$totalsAccuracy`; `PaymentReminderLevel`/`PaymentReminderChannel`, reused by `PaymentReminder`; `StatementEntryType`, reused by `StatementEntry`; `CreditNoteReasonCode`/`CreditNoteDisposition`, reused by `CreditNote`.
