...
- **the sum checks out by eye**: `537.6 + 1193.775 = 1731.375`, the note's `weight` — and `1.176 + 2.236 = 3.412`, its `volume`;
- **what a line weighs and occupies is what left**: the first delivered 84 of the 120 square meters ordered, and both its measures are those of the 84.
...
| <a id="goodsreceiptline"></a>`GoodsReceiptLine` | `StructuredValue` | A `GoodsReceiptConfirmation` line: `position`, `item`, `expectedQuantity`/`receivedQuantity`, `condition` (state of the received goods), `discrepancyNote`, `weight`/`volume`. The buyer-side counterpart of `DeliveryLine`. 🔑 **Everything it measures describes what was received**, never what the delivery announced: `receivedQuantity` against `expectedQuantity`, and the `weight`/`volume` it takes from [`HasPhysicalMeasures`](../../../src/xyz/oihana/schema/traits/HasPhysicalMeasures.php) — the same trait `DeliveryLine` composes, which is precisely **what makes the two figures comparable**. A receipt weighing less than the note announced is exactly the discrepancy this class exists to record. |
...
- [`xyz\oihana\schema\enumerations`](../../../src/xyz/oihana/schema/enumerations) — `BusinessDocumentAuthority`, reused by `BusinessDocument::$authority`; `DocumentTotalsAccuracy`, reused by `BusinessDocument::$totalsAccuracy`; `QuantityOrigin`, reused by `BusinessDocumentLine::$quantityOrigin`; `PaymentReminderLevel`/`PaymentReminderChannel`, reused by `PaymentReminder`; `StatementEntryType`, reused by `StatementEntry`; `CreditNoteReasonCode`/`CreditNoteDisposition`, reused by `CreditNote`.
