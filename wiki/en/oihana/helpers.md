...
| `hydrateApplicableResource` | `ApplicableResource` | `item` — typed down to a `Product`, an already typed resource being left alone rather than rebuilt. Accepts an instance as well as an array ; returns `null` on anything else. A link **with no resource** is still a link : the source may name a rank and nothing else. Reserved for the **constructor** path : `Reflection::hydrate()` types the entries on its own, the attribute being declared on `Product::$hasApplicableResource`. |
