...
The SKOS relations (`broader`, `narrower`, `related`, the `*Match` mappings, `topConceptOf`, `hasTopConcept`, `member`, `memberList`) are **traversal-only**: they are never persisted and never harvested — they are populated only on selected API responses (e.g. `/{key}/children`, `/descendants`, `/ancestors`).

Each relation is a **resolved reference**: it accepts a hydrated object, a scalar reference (a bare `_key`), **or** a raw associative `array` (an AQL-projected document, rebuildable via `new Concept($array)`). Plural relations are hydrated element by element through `#[HydrateWith(Concept::class)]` — but only on the reflection-based hydration path, not the lightweight constructor (which copies values verbatim). Collection `member`/`memberList` go one step further with `#[HydrateWith(Concept::class, Collection::class)]`: each entry is dispatched **polymorphically** on its `@type` discriminator (falling back to `Concept`).
...
([
    ...
    Collection::MEMBER =>
    [
        [ '@type' => 'Concept'    , 'name' => 'Merlot' ] ,         // hydrates as Concept
        [ '@type' => 'Collection' , 'name' => 'Sparkling whites' ] , // hydrates as Collection
    ],
]);
```
