...
- [Architecture](architecture.md) — the library's two storeys in three figures measured from the code: the layers, the Schema.org types that carry the Oihana layer, the couplings between domains.
...
| [`xyz\oihana\schema\appointments`](oihana/appointments.md) | [Appointments](oihana/appointments.md) | A meeting arranged with a customer and what gets written about it: `CustomerAppointment` (an `Event` — two axes of state, the slot and the meeting), `VisitReport` (mood, outcome, topics), `FollowUp` (what is still owed, by when). |
