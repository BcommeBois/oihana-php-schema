...
| [Documents commerciaux](business-documents.md) | `…\business\documents` | Le cycle devis → bon de commande → facture : `BusinessDocument`, `Quote`, `PurchaseOrder`, plus les objets de valeur transverses `TaxDetail`, `Adjustment`, `EcoFeeRule`/`AppliedEcoFee`, `DocumentTotals`, `BusinessDocumentLine`, `PaymentSchedule`/`PaymentInstallment`. |
