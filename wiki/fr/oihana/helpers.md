...
| `hydrateApplicableResource` | `ApplicableResource` | `item` — typé en `Product`, une ressource déjà typée étant laissée telle quelle plutôt que reconstruite. Accepte aussi bien une instance qu'un tableau ; rend `null` sur tout le reste. Un lien **sans ressource** reste un lien : la source peut ne nommer qu'un rang. Réservé au chemin du **constructeur** : `Reflection::hydrate()` type les entrées tout seul, l'attribut étant déclaré sur `Product::$hasApplicableResource`. |
