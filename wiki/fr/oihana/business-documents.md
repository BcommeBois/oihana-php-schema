...
| <a id="businessdocumentline"></a>`BusinessDocumentLine` | `StructuredValue` | Une ligne de document (`item`, `position`, `quantity`, `unit`, `price`, `taxes`, `adjustments`, `subtotal`, `total`, `additionalProperty`) — `taxes` et `adjustments` sont scopés à la ligne, un document peut donc mélanger des lignes à taux de TVA différents. `additionalProperty` (paires `PropertyValue`, hydratées par `Reflection::hydrate()`) porte les caractéristiques hors vocabulaire — numéro de lot, numéro de série, indicateur métier propre à l'ERP. |
