...
Cette page documente l'intégralité du namespace : les **objets de valeur transverses** (`TaxDetail`, `Adjustment`, `PaymentReminder`, `DeliveryLine`, `ProofOfDelivery`…), la **hiérarchie de documents** complète (`BusinessDocument`, `Quote`, `PurchaseOrder`, `Invoice`, `CreditNote`, `DeliveryNote`, `Receipt`, `Statement`) et l'**export** (`BusinessDocumentExporter`, `JsonLdExporter`).
