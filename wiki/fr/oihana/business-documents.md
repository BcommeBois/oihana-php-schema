...
Cette page documente à la fois les **objets de valeur transverses** (`TaxDetail`, `Adjustment`…) et le socle de la **hiérarchie de documents** (`BusinessDocument`, `Quote`, `PurchaseOrder`). Restent à venir dans une prochaine version : `Invoice`, `CreditNote`, `DeliveryNote`, `Receipt`, `Statement` et l'interface d'export ; cette page sera complétée en conséquence.
