...
| <a id="businessdocumentline"></a>`BusinessDocumentLine` | `StructuredValue` | Une ligne de document (`item`, `position`, `quantity`, `unit`, `price`, `taxes`, `adjustments`, `subtotal`, `total`, `additionalProperty`, `color`) — `taxes` et `adjustments` sont scopés à la ligne, un document peut donc mélanger des lignes à taux de TVA différents. `additionalProperty` (paires `PropertyValue`, hydratées par `Reflection::hydrate()`) porte les caractéristiques hors vocabulaire — numéro de lot, numéro de série, indicateur métier propre à l'ERP. `color` (composée depuis `HasColor`, la même indication maison `#RRGGBB` que portent les familles du thésaurus et `ProductType`) permet de teinter la ligne pour sa propre raison, sans remonter à l'item puis à son type. |
