...
### Un bon de livraison moissonné, en entier

Cet exemple montre en situation ce qu'aucune description ne rend aussi vite : **deux lignes portant la même `position`, deux commandes, deux factures.**
  ...
  "lines": [
    {
      ...
      "volume": { "value": 1.176, "unitCode": "MTQ", "unitText": "Cubic Meter" }
    },
    {
      "@type": "DeliveryLine",
      "position": 1,
      ...
      "volume": { "value": 2.236,    "unitCode": "MTQ", "unitText": "Cubic Meter" }
    }
  ]
}
```

Trois lectures que le document rend évidentes :

- **la `position` seule ne nomme rien.** Les deux lignes portent la même, et ce sont deux commandes différentes — d'où `referencesOrder` sur chacune, et `referencesInvoice` qui suit la même logique : le bon est facturé par **deux** factures, une par commande, ce qu'une référence posée sur l'en-tête ne saurait dire ;
- **la somme se vérifie à l'œil** : `537,6 + 1 193,775 = 1 731,375`, le `weight` du bon — et `1,176 + 2,236 = 3,412`, son `volume` ;
- **ce qu'une ligne pèse et occupe est celui de ce qui est parti** : la première a livré 84 des 120 m² commandés, ses deux mesures sont celles des 84.
...
| <a id="adjustment"></a>`Adjustment` | `StructuredValue` | Un ajustement de prix (`type`, `amount` ou `percentage`, `reason`, `includedInBase`, `includedInTotal`, `taxes`), inspiré d'UBL `AllowanceCharge`. Couvre remise, majoration, frais de port, éco-participation, consigne, emballage via la seule propriété `type` (voir `PriceComponentType`). `includedInTotal` dit si l'ajustement **compte dans les totaux du document** — **son absence veut dire qu'il compte**, seul un ajustement écarté le déclare, à `false`. ⚠️ À ne pas confondre avec `includedInBase`, qui répond à tout autre chose : celui-là dit que le montant est **déjà compris** dans le prix de base au lieu de s'y ajouter ; celui-ci dit s'il atteint les totaux. Un ajustement peut parfaitement s'ajouter au prix **et** rester hors du total — une source affiche parfois un frais sur un document en refusant de l'imputer tant qu'une option n'est pas tranchée, et l'additionner surestimerait ce qui est dû. 🔑 **Même nom, même défaut et même sens que `BusinessDocumentLine::$includedInTotal`** : un drapeau qui s'appellerait autrement au motif qu'il vit sur une autre classe serait une occasion manquée, un consommateur apprenant la règle une seule fois. Un ajustement à effet monétaire peut porter la taxe qu'il doit en propre, à un taux qui lui est propre : la TVA d'un port répond au transporteur, pas à la marchandise livrée. |
| <a id="ecofeerule"></a>`EcoFeeRule` | `StructuredValue` | La règle de calcul d'une contribution environnementale (`category`, `rate`, `validFrom`, `validThrough`) — un concept de catalogue, sans effet monétaire propre. `rate` est un `UnitPriceSpecification` et **non** un `MonetaryAmount` : une contribution se calcule sur une grandeur physique — un poids, une surface, un volume, un nombre de pièces — jamais sur un prix, donc l'unité n'est pas un ornement, c'est la moitié de la règle. Un `MonetaryAmount` ne saurait dire que « 215 € » là où la règle dit « 215 € **la tonne** ». `category` porte ce que la règle vise : le nom dit catégorie parce que c'est le cas courant, mais l'union accepte n'importe quel `Thing` — un produit compris — et une règle attachée à un seul article est aussi légitime qu'une règle attachée à une famille ; une source qui publie ses barèmes article par article n'est pas une exception à contourner. Une chaîne nue le désigne par référence, sans joindre la fiche. |
...
| <a id="businessdocumentline"></a>`BusinessDocumentLine` | `StructuredValue` | Une ligne de document (`item`, `position`, `quantity`, `quantityOrigin`, `unit`, `price`, `taxes`, `adjustments`, `subtotal`, `total`, `additionalProperty`, `color`, `freeReason`, `includedInTotal`, `section`, `technicalNote`, `volume`, `weight`) — `taxes` et `adjustments` sont scopés à la ligne, un document peut donc mélanger des lignes à taux de TVA différents. `additionalProperty` (paires `PropertyValue`, hydratées par `Reflection::hydrate()`) porte les caractéristiques hors vocabulaire — numéro de lot, numéro de série, indicateur métier propre à l'ERP. `color` (composée depuis `HasColor`, la même indication maison `#RRGGBB` que portent les familles du thésaurus et `ProductType`) permet de teinter la ligne pour sa propre raison, sans remonter à l'item puis à son type. `freeReason` (→ `DefinedTerm`) dit pourquoi la marchandise part sans être facturée — geste commercial, casse, échantillon, marchandise déjà au client : **c'est sa présence qui dit que la ligne est offerte**, aucun booléen ne l'accompagne, un drapeau et un motif tenus en parallèle finissant toujours par diverger. Le terme est figé par son code et son libellé (`id` + `name`), jamais par sa clé de stockage, qu'une réharvest du vocabulaire renumérote. `technicalNote` est le pendant de la `description` héritée : celle-ci s'adresse au client, celle-là à qui prépare la marchandise (« reprendre les 3 colis palette 12, ne pas remettre en stock ») — les garder séparées est ce qui permet d'imprimer un même document deux fois, pour deux publics. Elle se sérialise comme toute autre propriété : c'est au rendu client de l'omettre, le type ne le fait pas à sa place. `includedInTotal` dit si la ligne compte dans les totaux du document — **son absence veut dire qu'elle compte**, seule une ligne écartée le déclare, à `false`. Une ligne chiffrée n'est pas toujours une ligne à payer : un devis peut proposer deux fois le même ouvrage — deux parquets, deux finitions — et attendre que le client en retienne un ; les deux sont imprimés, les deux sont chiffrés, un seul est facturé. Les lignes de l'option écartée sont des lignes ordinaires, avec un article, une quantité et un prix : rien dans leur contenu ne les distingue, et les additionner malgré tout a été mesuré comme doublant un devis réel. La même case sert à tout ce qui s'affiche sans être dû — un frais indicatif, un montant cité pour mémoire ; à ne pas lire comme « cette ligne est une variante », la raison de l'écart ne la regarde pas, seul le fait compte. `section` porte l'intitulé du chapitre auquel la ligne appartient — « charpente », « parquet chêne », « pose » — un simple libellé répété sur chaque ligne du même groupe, et volontairement rien de plus : ni classe de section, ni imbrication, ni sous-total propre. Le regroupement est ce qui partage le libellé, ce qui suffit à imprimer un document en chapitres et coûte assez peu pour valoir d'être porté même quand rien ne le lit encore. 🚨 **Ne jamais y ranger le sous-total du groupe** : un récapitulatif posé à côté des lignes qu'il récapitule est le plus court chemin pour compter deux fois la même marchandise — il est dérivé, et il le reste. À distinguer de la `description` héritée, qui nomme l'article : une description appartient à une ligne, un intitulé est partagé par plusieurs. `quantityOrigin` (→ `QuantityOrigin`) dit **d'où vient la quantité** de la ligne : calculée par le système, ou saisie par quelqu'un. La plupart des lignes sont saisies et la question ne se pose pas ; elle se pose dès qu'une ligne est écrite **en conséquence d'une autre** — une prestation qui accompagne la marchandise, un service que l'article porte — car sa quantité est alors déduite de la ligne qu'elle sert, et les deux sont liées : quatre planches de 0,019 mètre cube font 0,076 mètre cube de traitement, et rien d'autre. 🚨 **Deux règles sont justes et elles s'excluent** : une quantité liée qui cesse de suivre sous-facture en silence — passer les planches de quatre à cent en laissant le traitement à 0,076 facture vingt-cinq fois moins de travail qu'il n'en sera fait, sans que rien à l'écran ne le signale ; une quantité saisie qu'on réécrit efface une décision tout aussi discrètement, car celui qui a tapé le chiffre avait mesuré, ou convenu d'un forfait. La ligne doit donc se souvenir de laquelle des deux elle relève, et c'est ici qu'elle le fait : elle naît `CALCULATED` et passe `ENTERED` au premier chiffre tapé ; **elle n'en revient pas d'elle-même** — revenir à la valeur calculée est un geste en soi, qu'un producteur est censé offrir, et vider la quantité en est la demande d'usage. ⚠️ **Une valeur absente ne dit rien** et ne doit pas se lire `ENTERED` : une ligne écrite avant l'arrivée de la propriété ne dit rien de l'origine de son nombre. 🔑 **Elle dit d'où vient le nombre, jamais à quoi la ligne est liée** : la ligne suivie — quand il y en a une — est nommée par l'`isPartOf` hérité, et les deux répondent à deux questions différentes, une provenance et une relation. `weight` et `volume` (composés depuis [`HasPhysicalMeasures`](../../../src/xyz/oihana/schema/traits/HasPhysicalMeasures.php)) disent ce que la ligne pèse et ce qu'elle occupe — la quantité par ce que pèse et ce qu'occupe l'unité dans laquelle elle est comptée ; la somme, sur les lignes, redonne les valeurs de l'en-tête. Un nombre nu quand l'unité est implicite, un `QuantitativeValue` quand elle est déclarée. La paire est déclarée **une seule fois**, dans le trait, plutôt que recopiée sur chaque classe de ligne : la même union, le même `#[HydrateAs]`, la même lecture — écrites trois fois, elles auraient divergé. |
...
| <a id="goodsreceiptline"></a>`GoodsReceiptLine` | `StructuredValue` | Une ligne de `GoodsReceiptConfirmation` : `position`, `item`, `expectedQuantity`/`receivedQuantity`, `condition` (état des biens reçus), `discrepancyNote`, `weight`/`volume`. Le pendant acheteur de `DeliveryLine`. 🔑 **Tout ce qu'elle mesure décrit ce qui a été reçu**, jamais ce que la livraison annonçait : `receivedQuantity` face à `expectedQuantity`, et le `weight`/`volume` qu'elle tient de [`HasPhysicalMeasures`](../../../src/xyz/oihana/schema/traits/HasPhysicalMeasures.php) — le même trait que `DeliveryLine`, ce qui est précisément **ce qui rend les deux chiffres comparables**. Une réception qui pèse moins que le bon ne l'annonçait est exactement l'écart que cette classe existe pour consigner. |
...
- [`xyz\oihana\schema\enumerations`](../../../src/xyz/oihana/schema/enumerations) — `BusinessDocumentAuthority`, réutilisée par `BusinessDocument::$authority` ; `DocumentTotalsAccuracy`, réutilisée par `BusinessDocument::$totalsAccuracy` ; `QuantityOrigin`, réutilisée par `BusinessDocumentLine::$quantityOrigin` ; `PaymentReminderLevel`/`PaymentReminderChannel`, réutilisées par `PaymentReminder` ; `StatementEntryType`, réutilisée par `StatementEntry` ; `CreditNoteReasonCode`/`CreditNoteDisposition`, réutilisées par `CreditNote`.
