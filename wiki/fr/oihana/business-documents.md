...
| <a id="businessdocument"></a>`BusinessDocument` | `Intangible` | Le parent commun du cycle devis → commande → facture : `adjustments` (ajustements de niveau document, cf. `Adjustment`), `attachments`, `billingAddress`, `contact`, `currency`, `customer`, `documentLines`, `issueDate`, `orderDelivery`, `paymentTerms`, `pointOfSale`, `references`, `seller`, `status` (→ `BusinessDocumentStatus`), `taxes`, `totals`. Les quatre propriétés d'en-tête `billingAddress` (`PostalAddress` — l'adresse de facturation), `contact` (`Person` — l'interlocuteur, absent du vocabulaire Schema.org), `orderDelivery` (`ParcelDelivery` — l'adresse de livraison, le mode et la date souhaitée) et `pointOfSale` (`Place` — le point de vente qui porte le document) complètent les parties (`customer`/`seller`) et se recopient figées, pour que le document se suffise à lui-même même si les fiches évoluent ensuite. Étend `Intangible` plutôt que de réutiliser `org\schema\Order`/`org\schema\Invoice` : un document commercial qualifie une transaction, ce n'est pas une ressource adressable en propre — et cela laisse le miroir Schema.org intact (les consommateurs existants d'`org\schema\Order`/`Invoice` ne voient aucun changement). |
