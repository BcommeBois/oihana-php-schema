...
- [`xyz\oihana\schema\enumerations`](../../../src/xyz/oihana/schema/enumerations) — `BusinessDocumentAuthority`, réutilisée par `BusinessDocument::$authority` ; `DocumentTotalsAccuracy`, réutilisée par `BusinessDocument::$totalsAccuracy` ; `PaymentReminderLevel`/`PaymentReminderChannel`, réutilisées par `PaymentReminder` ; `StatementEntryType`, réutilisée par `StatementEntry` ; `CreditNoteReasonCode`/`CreditNoteDisposition`, réutilisées par `CreditNote`.
