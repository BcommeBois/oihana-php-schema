...
| [`xyz\oihana\schema\appointments`](oihana/appointments.md) | [Rendez-vous](oihana/appointments.md) | Une rencontre convenue avec un client et ce qu'on en écrit : `CustomerAppointment` (un `Event` — deux axes d'état, le créneau et la rencontre), `VisitReport` (climat, résultat, sujets), `FollowUp` (ce qui reste dû, pour quand). |
...
| [`xyz\oihana\schema\shipping`](oihana/shipping.md) | [Expédition Oihana](oihana/shipping.md)  | Dispositions d'expédition permanentes : `DeliveryRouteAssignment` — quelle tournée dessert une adresse, quels jours. |
| [`xyz\oihana\schema\statistics`](oihana/statistics.md) | [Statistiques Oihana](oihana/statistics.md) | Des corps de chiffres déjà mesurés : `Statistics`, `ObservationSeries`, `CustomerStatistics`, `ProviderStatistics` — ce qu'un partenaire a échangé sur une année, mois par mois. |
| [`xyz\oihana\schema\thesaurus`](oihana/thesaurus.md) | [Thésaurus Oihana (SKOS)](oihana/thesaurus.md) | Arbres de concepts SKOS : `ThesaurusTerm`, `Concept`, `ProductCategoryTerm`, `ConceptScheme`, `Collection`, `OrderedCollection` — hiérarchie, notes, alignements, collections — le `DeliveryMethodTerm` tarifé et le `DeliveryRouteTerm` récurrent, plus la couche registre (`ThesaurusScheme`, `ThesaurusDomain`). |
