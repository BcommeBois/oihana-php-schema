...
| [`xyz\oihana\schema\appointments`](oihana/appointments.md) | [Rendez-vous](oihana/appointments.md) | Une rencontre convenue avec un client et ce qu'on en écrit : `Appointment` (un `Event` — deux axes d'état, le créneau et la rencontre), `VisitReport` (climat, résultat, sujets), `FollowUp` (ce qui reste dû, pour quand). |
