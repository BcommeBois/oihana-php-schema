...
- Adds the `xyz\oihana\schema\business` namespace and the `BusinessIdentity` entity (extends `Intangible`) — the typed link between an authenticated account and a business entity (a `Person` or an `Organization`), with `role`, `subject` and `memberOf` properties. Keeps the account and its linked entity decoupled (no data merging); `subject`/`memberOf` are resolved references, never copies.
  ...
  - Adds neutral read accessors so consumers can navigate the link without re-implementing the lookup logic : `BusinessIdentity::is()` (role test, tolerant when `role` is undefined), `BusinessIdentity::subjectId()` and `BusinessIdentity::memberOfId()` (resolve the `id`/`_key`/`_id` of a scalar or `Thing` reference), plus `User::identitiesByRole()` and `User::firstIdentityByRole()` to filter the account's identities by role.
  ...
  - Adds the `UserProfile` entity (extends `Intangible`) — a creation-time template that pairs an authorization `role` with the `expectedType` (`additionalType`) of the person the account will be linked to, plus a `color` UI hint. It carries no per-account state : it is consumed once, at creation time, to grant a role and attach a business identity.
  - Adds the companion `xyz\oihana\schema\constants\traits\business\UserProfileTrait` trait centralising the `UserProfile` property name constants (`COLOR`, `EXPECTED_TYPE`, `ROLE`). Like `BusinessIdentityTrait`, it is intentionally **not** aggregated into `xyz\oihana\schema\constants\Oihana` (its `ROLE` key collides with `BusinessIdentityTrait::ROLE`); both are composed into the `xyz\oihana\schema\constants\traits\BusinessTrait` aggregator instead.
